...
\cs_set:Npn \__emoji_def:nnnn
  {
    ...
    \__emoji_def_aux:nnnn
  }
\cs_set:Npn \__emoji_def_aux:nnnn #1#2#3#4
  {
      \emoji {#2}
    & \ttfamily \scriptsize #2
    & \ttfamily \scriptsize
      \clist_use:nn {#3} { , ~ }
    & \scriptsize #4
    & \ttfamily \scriptsize
...
\cs_set:Npn \__emoji_doc_begin_longtable:n #1
  {
    \begin{longtable} { C{1.2cm} P{4cm} P{2.8cm} P{4cm} C{1.8cm} }
        \caption{#1} \\
